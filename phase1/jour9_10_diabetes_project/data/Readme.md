...
- `data/test_without_class.csv` pour le set de test dont la variable `class` a été supprimée.  

## Application d'évaluation du scoring  

Une application développée en `shiny python` et déployée sur la plateforme [posit connect cloud](https://connect.posit.cloud/) a été spécialement développée pour tester vos prédictions sur le set de test *caché*:  
